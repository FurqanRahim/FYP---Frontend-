...
const LowAttendance = ({ teacherId }) => {
  ...
  useEffect(() => {
    setIsLoading(true);
    axios
      .get(`http://localhost:8080/api/teacher/${teacherId}/sessions`)
      .then((res) => {
  ...
  useEffect(() => {
    if (selectedSession) {
      setIsLoading(true);
      axios
        .get(`http://localhost:8080/api/teacher/${teacherId}?session=${selectedSession}`)
        .then((res) => {
  ...
  useEffect(() => {
    if (selectedCourse && selectedClass) {
      setIsLoading(true);
      axios
        .get(`http://localhost:8080/api/enrolled-student?courseId=${selectedCourse}&classId=${selectedClass}`)
        .then((res) => {
      ...
      axios
        .get(`http://localhost:8080/api/low-attendance`,{
